...
public class DungeonPartitionTree {
    ...
    // Try to spawn a boss monster in a specific room (The exit staircase room)
    void spawnBossMonster(int[][] level_tile_map, ArrayList<Monster> monsters, ArrayList<Interactable> level_interactables, int dungeon_level, Random rand) {
        int monster_level = calculateLevel(rand) + dungeon_level + 1;
        int[] monster_spawn_location;
        if (this.territory_root.getTerritoryType()) {
            // Try to spawn one to four goblins
            for (int i = 0; i < 2; i++) {
